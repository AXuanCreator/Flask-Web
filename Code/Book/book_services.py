...
class BookServices:
    ...
    def insert_book(book_request):
        ...
        title, author, category_id, publisher, quantity = (
            book_request.get(key) for key in request_keys)

        if any(var is None for var in [title, author, category_id, publisher, quantity]):
            return ReturnCode.FAIL

        insert_book = Book(title=title, author=author,
                           category_id=category_id, publisher=publisher, quantity=quantity)
    ...
    def list_book(keywords, page, per_page):
        ...
        if 'publisher' in keywords:
            query = query.filter(Book.publisher.like(
                f"%{keywords['publisher']}%"))
    ...
    def recommend_book(id):
        """推荐书籍"""
        borrow_list = [
            borrow.book_id for borrow in Borrow.query.filter_by(user_id=id)]
        book_list = [book.id for book in Book.query.order_by(
        ...
        recommend_list = [book_id for book_id, rating in zip(
            book_list, recommend_list) if rating > 3.7]    # TODO : 为什么是3.7为分界点呢，因为模型有点问题，收敛3.7左右

...
def load_rating_with_csv():
	book_rating_csv = pd.read_csv('./Config/csv/goodread_dataset/ratings.csv')  # 若无法找到路径，则表示没有将Code设置为源代码目录

	with app.app_context():
		for index, row in book_rating_csv.iterrows():
			# print(f'{row}')
			book_rating = BookRating(user_id=row['user_id'], book_id=row['book_id'], rating=row['rating'])
		...
		for index, row in tqdm(borrow_csv.iterrows(), desc='Load Borrow'):
			try:
				borrow = Borrow(user_id=row['user_id'], book_id=row['book_id'],
				                borrow_date=datetime(random.randint(2018, 2023), random.randint(1, 12), random.randint(1, 28)),
				                return_date=datetime(2024, random.randint(1, 5), random.randint(1, 28)),
	...
	with app.app_context():
		for i in tqdm(range(nums), desc="Create User Data"):
			user = User(username=Helper.generate_random_string_with_pattern(Regex.username_pattern, 6, 10),
			            password=Helper.generate_random_string_with_pattern(Regex.password_pattern, 9, 19),
			            name=Helper.generate_random_string_with_choice(Regex.name_pattern, 2, 4),
			            gender=random.choice(['男', '女', 'Lady', 'Man']),
			            phone=Helper.generate_random_string_with_choice(string.digits, 11, 11),
			            email=Helper.generate_random_string_with_choice(string.digits, 7, 15) + '@' +
			                  random.choice(['qq.com', '163.com', 'gmail.com', 'outlook.com', 'mail.com']),

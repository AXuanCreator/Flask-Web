...
class UserDao:
    ...
    def update_user_id(id, user_request):
        ...
        # 使用列表推导式赋值
        request_keys = ['username', 'name',
                        'password', 'gender', 'phone', 'mail']
        username, name, password, gender, phone, mail = (
            user_request.get(key) for key in request_keys)
    ...
    def insert_user(user_request, commit=True):
        result_keys = ['username', 'password',
                       'name', 'gender', 'phone', 'mail']
        username, password, name, gender, phone, mail = (
            user_request.get(key) for key in result_keys)
...
class UserServices:
    ...
    def login(username, password):
        # username为Unique，所以不会出现查询到多条记录的情况
    ...
    def send_mail(mail):
        ...
        # 将验证码过时加入到计时器中
        scheduled_task(SendMail.remove_code,
                       UserConfig.MAIL_CODE_OUTTIME, mail)

...
def user_login():
    if request.method == 'GET':
        ...
        match UserServices.login(username, password):
            ...
            case _:
                print(
                    '\033[34m[WARN]\033[0m | Controller-->Login | Unexpected Output')
...
def user_register():
    ...
    if request.method == 'GET':
        if session.get('register') is not None and session['register']:
            match UserServices.register(session, commit=True):
                case ReturnCode.SUCCESS:
                    # 清空Session中的注册信息
                    register_keys = ['username', 'name', 'gender',
                                     'phone', 'mail', 'password', 'register']
                    for key in register_keys:
                        del session[key]
                    print(
                        '\033[35m[DEBUG]\033[0m | Register GET | Session Clear : ', session)
    ...
    # POST请求则用于将Register的页面信息存入Session里
        ...
        print(
            '\033[35m[DEBUG]\033[0m | Register POST Before Loading | Session : ', session)
        ...
        print(
            '\033[35m[DEBUG]\033[0m | Register POST After Loading | Session : ', session)
        # GET请求，因为从Session中获取信息
...
def user_send_code():
    ...
    if request.method == 'GET':
        ...
        match UserServices.send_mail(mail):
            case ReturnCode.SUCCESS:
                # 由于该URL带有参数，需要显式传入
                return redirect(url_for('user.user_verify_code', mail=mail))
            case ReturnCode.FAIL:
                return Response.response(ResponseCode.FAILED, 'Send Mail FAIL', None)
            case ReturnCode.MAIL_NOT_ALLOWED:
                return Response.response(ResponseCode.FAILED, 'Mail Wrong', None)
...
def user_verify_code(mail):
    if request.method == 'GET':
        ...
        match UserServices.check_code(mail, request.form.get('code'), need_user_check=False):
            case ReturnCode.SUCCESS:
                print(
                    '\033[35m[DEBUG]\033[0m | Verify Code POST | Session : ', session)

                session['user_login'] = User.query.filter_by(
                    mail=mail).first().username  # 验证状态——通过
